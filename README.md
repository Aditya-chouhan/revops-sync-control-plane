...
**Upgrade before running current delivery workers:** stop all older delivery processes, run `alembic upgrade head` (revision `0005`), then restart with the new code. Revision `0004` conservatively holds legacy `dispatching`, `outcome_unknown`, and `reconciled` items; revision `0005` adds nullable dispatch-binding fields without rewriting old outcomes or staged intent. `AUTO_CREATE_SCHEMA` does not upgrade an existing table. `OUTBOX_CLAIM_SECONDS` defaults to 300; synchronize worker clocks and size the lease above expected I/O latency. Expiry does not cancel an HTTP call already in flight and never authorizes a replacement write.

Before a fresh or safe-retry dispatch, [target binding](docs/TARGET_BINDING.md) checks acknowledged IDs in the same stream. A queued POST becomes an effective PATCH when an earlier operation confirmed one CRM object. Original staged operation, payload, checksum, and idempotency key remain unchanged; dispatch method, target, and source item are persisted separately under the worker fence. Missing or conflicting acknowledged IDs stop delivery before HTTP.

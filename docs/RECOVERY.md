...
5. If one object matches every intended field, read-back acknowledges it automatically but preserves its ordering hold. Newer items remain blocked. Once the provider operation is established as settled and no claim remains, a trusted operator can record an audited hold release using [the internal utility](ORDERING.md). Matching fields alone are insufficient. If duplicates exist, stop for a reviewed identity resolution. Do not delete records automatically.
...
7. If investigation establishes that a new write is necessary, record the investigation and obtain operator approval. Simply staging a fresh operation cannot bypass an unknown or rejected predecessor. This repository does not provide an automated "force replay", cancellation, or dead-letter resolution endpoint; hold release cannot fabricate acknowledgement.
...
pytest tests/test_stream_ordering.py -v
...
Same-item exclusion is database-backed and token-fenced, with read-only expired-claim takeover. Different items now follow the full-prefix stream gate described in [ordering](ORDERING.md): older items must be acknowledged, unclaimed, and free of uncertainty holds. This does not cancel provider requests already in flight or enforce provider-side conditional updates or per-field version contracts against external edits. Matching read-back does not prove the absence of unrelated duplicates outside the lookup result or establish exactly-once side effects. Settlement release records a trusted operator attestation, not provider verification. The new concurrency and migration evidence is SQLite-only; no new live-provider or PostgreSQL verification is claimed. No public delivery endpoint, background worker service, webhook activation, or paid/provider credential use was introduced.

...
Delivery additionally requires a database-backed worker claim. Acquisition is a conditional UPDATE against stored state, independent of ORM cache contents, with a correlated check that every older item for the same canonical account/provider is acknowledged, unclaimed, and free of an ordering hold. Every transition is fenced by a unique token and lease expiry; losing workers make no HTTP request, and expired claims permit only read-back. Current revision `0005` must be migrated with old workers stopped. Uncertain writes keep the stream held even after matching read-back until an audited operator settlement release. Before fresh/safe-retry dispatch, [target binding](TARGET_BINDING.md) propagates one acknowledged CRM ID into a separately persisted PATCH plan; conflicting or missing historical identity blocks HTTP. This does not enforce provider-side conditional updates; see [ordering boundaries](ORDERING.md).

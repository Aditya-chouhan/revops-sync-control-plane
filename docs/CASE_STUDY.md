...
The regression suite simulates accepted writes with lost responses, reopens the database, and checks that uncertain items perform only read-back while acknowledged items perform no HTTP request. These are synthetic software results, not new live CRM evidence or an exactly-once guarantee. Concurrent-worker claims and conditional updates remain separate gaps; see [operator recovery](RECOVERY.md).

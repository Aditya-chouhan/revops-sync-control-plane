...
The regression suite simulates accepted writes with lost responses, reopens the database, and checks that uncertain items perform only read-back while acknowledged items perform no HTTP request. I then added database-backed worker claims: two stale worker snapshots race an atomic conditional UPDATE, and only the winning claim can dispatch. Token-fenced state changes prevent a stale holder from overwriting or clearing a successor's result. Lease takeover is always read-back only, including after a crash before dispatch. These are synthetic software results, not new live CRM evidence or an exactly-once guarantee. Ordering across distinct items and conditional updates remain gaps; see [operator recovery](RECOVERY.md).

...
The delivery client originally retried transport timeouts and 5xx responses while merely sending an `Idempotency-Key` header. Neither a timeout nor that header proves the CRM did not commit a write. I added a committed pre-dispatch marker and a read-back policy: exact object ID for updates, unique canonical-stamp lookup for creates, then identity and every intended field compared against provider state. Matching state is acknowledged as `reconciled`; missing, mismatched, duplicate, or unreadable state remains `outcome_unknown` without another write.

The regression suite simulates accepted writes with lost responses, reopens the database, and checks that uncertain items perform only read-back while acknowledged items perform no HTTP request. I then added database-backed worker claims: two stale worker snapshots race an atomic conditional UPDATE, and only the winning claim can dispatch. Token-fenced state changes prevent a stale holder from overwriting or clearing a successor's result. Lease takeover is always read-back only, including after a crash before dispatch. Next, the atomic claim gained a full older-prefix gate per account/provider. Ambiguous writes retain a persistent hold even after matching read-back, because a late remote request could still apply. Releasing that hold requires explicit operator settlement attestation and an atomic audit record. Different-item races and a still-running expired request are covered with independent SQLite sessions and simulated HTTP. These are synthetic software results, not new live CRM evidence or an exactly-once guarantee. Provider-side conditional updates remain a gap; see [operator recovery](RECOVERY.md) and [stream ordering](ORDERING.md).

## What I would add with an authorized environment

Ingestion had a separate correctness gap: delayed snapshots could replace newer source data before field ownership/consent reconciliation. I added version checks before identity resolution, explicit UTC normalization, stale receipts, and atomic rejection of equal-version conflicting snapshots. Tests cover all six serial arrival orders of three versions and stale domain/hint/consent protection. This is normalized-snapshot software evidence, not signed webhook support or concurrent ingestion exclusion; see [ingestion ordering](INGESTION_ORDERING.md).
